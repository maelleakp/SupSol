...
if (isset($_POST["submit"])){
    if (!empty($_POST["tel"]) && !empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["mail"]) && !empty($_POST["password"]) && !empty($_POST["password2"])){
        ...
        if ($idV == NULL) {
            $sql = "INSERT INTO User(NOM_U, PRENOM_U, MAIL_U, MDP_U, TEL_U) VALUES ('$nom','$prenom','$mail','$password', '$tel')";
            $dbh->query($sql); 
