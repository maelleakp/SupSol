...
if (isset($_POST['submit'])){
    ...
    if ($id == NULL) {
        ...
        $dbh->query($insert);        
        echo "<script>alert('Your participation has been validated, thank you!');
        window.location.href = '../view/concours.php';</script>";        
    } else {
        echo "<script>alert('You have already participated.');
        window.location.href = '../view/concours.php';</script>";
    }
}
...
if (isset($_POST['elir'])){
    foreach ($_POST['elir'] as $key => $value){
        ...
        if ($id == NULL) { 
            header('Location: ../view/elirAnglais.php');
        }
        else {
            echo "<script>alert('You have already participated.');
            window.location.href = '../view/electionsAnglais.php';</script>";
        }   
    }
}
...
if (isset($_POST['vote'])){
    if (!empty($_POST['joueur']) && !empty($_POST['gardien'])){
        ...
        header('Location: ../view/electionsAnglais.php');
    }
    else {
        header('Location: ../view/elirAnglais.php');
    }
